...
export default function Overview() {
  function btnOverview() {
    setContent(Data[6].overview.content);
    setSource(Data[6].overview.source)
    setImg(Uranus)
    setGeology("")
    setActive("active-uranus")
  }

  function btnInternal() {
    setContent(Data[6].structure.content);
  ...
  function btnSurface() {
    setContent(Data[6].geology.content);
    setSource(Data[6].geology.source)
    setImg(Uranus)
    setGeology(UranusG)
  }

  const [content, setContent] = useState(Data[6].overview.content);
  const [source, setSource] = useState(Data[6].overview.source);
  const [img, setImg] = useState(Uranus);
  const [geology, setGeology] = useState<string>("")
  const [active, setActive] = useState<string>("active-uranus")
    ...
    <div>
      ...
      <div className="main">
        <div className="top">
          ...
          <div className="info">
            <h1 className="title">{Data[6].name.toUpperCase()}</h1>
            ...
            <div className="buttons-uranus-overview">
              <button id={source == Data[6].overview.source ? active : ""} onClick={btnOverview}>
                <p>01</p> <span>OVERVIEW</span>
              </button>

              <button id={source == Data[6].structure.source ? active : ""} onClick={btnInternal}>
                <p>02</p> <span>INTERNAL STRUCTURE</span>
              </button>

              <button id={source == Data[6].geology.source ? active : ""} onClick={btnSurface}>
        ...
        <div className="specifics">
          <div>
            <p>ROTATION TIME</p>
            <h1>{Data[6].rotation}</h1>
          </div>

          <div>
            <p>REVOLUTION TIME</p>
            <h1>{Data[6].revolution}</h1>
          </div>
