...
export default function Overview() {
  function btnOverview() {
    setContent(Data[1].overview.content);
    setSource(Data[1].overview.source)
    setImg(Venus)
    setGeology("")
    setActive("active-venus")
  }

  function btnInternal() {
    setContent(Data[1].structure.content);
  ...
  function btnSurface() {
    setContent(Data[1].geology.content);
    setSource(Data[1].geology.source)
    setImg(Venus)
    setGeology(VenusG)
  }

  const [content, setContent] = useState(Data[1].overview.content);
  const [source, setSource] = useState(Data[1].overview.source);
  const [img, setImg] = useState(Venus);
  const [geology, setGeology] = useState<string>("")
  const [active, setActive] = useState<string>("active-venus")
    ...
    <div>
      ...
      <div className="main">
        <div className="top">
          ...
          <div className="info">
            <h1 className="title">{Data[1].name.toUpperCase()}</h1>
            ...
            <div className="buttons-venus-overview">
              <button id={source == Data[1].overview.source ? active : ""} onClick={btnOverview}>
                <p>01</p> <span>OVERVIEW</span>
              </button>

              <button id={source == Data[1].structure.source ? active : ""} onClick={btnInternal}>
                <p>02</p> <span>INTERNAL STRUCTURE</span>
              </button>

              <button id={source == Data[1].geology.source ? active : ""} onClick={btnSurface}>
        ...
        <div className="specifics">
          <div>
            <p>ROTATION TIME</p>
            <h1>{Data[1].rotation}</h1>
          </div>

          <div>
            <p>REVOLUTION TIME</p>
            <h1>{Data[1].revolution}</h1>
          </div>
